...
function validateBody(body: Record<string, unknown>): { text: string; source: string; target: string } | string {
  ...
  if (typeof text !== "string" || !text.trim()) return "Missing or empty 'text' field";
  if (text.length > MAX_TEXT_LENGTH) return `Text exceeds maximum length of ${MAX_TEXT_LENGTH} characters`;
  if (typeof source !== "string" || !VALID_LANG_CODES.has(source)) return `Invalid 'source' language code: ${source}`;
  if (typeof target !== "string" || !VALID_LANG_CODES.has(target)) return `Invalid 'target' language code: ${target}`;
...
app.post("/api/translate/google", async (req, res) => {
  ...
  try {
    const response = await httpPost(
      ...
      JSON.stringify({ q: [validation.text], source: validation.source, target: validation.target })
    );
...
app.post("/api/translate/deepl", async (req, res) => {
  ...
  try {
    const response = await httpPost(
      ...
      { "Authorization": `DeepL-Auth-Key ${apiKey}`, "Content-Type": "application/json" },
      JSON.stringify({ text: [validation.text], source_lang: validation.source, target_lang: validation.target })
    );
